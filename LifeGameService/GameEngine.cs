...
namespace LifeGameService
{
    public class GameEngine
    {
        ...
        public void NextGeneration()
        {
            ...
            (this.newField, this.field) = (this.field, this.newField);
            this.CurrentGeneration++;
        }

        public byte[][] GetCurrentField() => this.field;

        public void AddCell(int x, int y) => this.UpdateCell(y, x, state: true);

        public void RemoveCell(int x, int y) => this.UpdateCell(y, x, state: false);

        private bool ValidateCellPosition(int i, int j) => i >= 0 && j >= 0 && i < this.rows && j < this.cols;
